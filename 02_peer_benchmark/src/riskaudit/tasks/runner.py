...
def _require_event_range(
    ...
    if actual_dates[0] > observation_start or actual_dates[1] < observation_end:
        raise ValueError(
            "风险事件批次未完整覆盖 02 年初至上传截止日范围；请先按 01 口径重跑。"
            f"事件批次={actual[0]}—{actual[1]}，02 计算范围={expected[0]}—{expected[1]}"
        )


def _event_observation_range(manifest_path: str | Path) -> tuple[date, date]:
    manifest = json.loads(Path(manifest_path).read_text(encoding="utf-8"))
    try:
        start = date.fromisoformat(str(manifest.get("observation_start") or ""))
        end = date.fromisoformat(str(manifest.get("observation_end") or ""))
    except ValueError as exc:
        raise ValueError("风险事件 manifest 缺少有效观察日期") from exc
    if start > end:
        raise ValueError("风险事件 manifest 观察日期范围无效")
    return start, end


def _history_observation_range(
    history_files: dict[str, str | Path],
) -> tuple[date, date]:
    from riskaudit.business_files import inspect_business_date_range

    ranges = [
        inspect_business_date_range(path)[:2] for path in history_files.values()
    ]
    if not ranges:
        raise ValueError("历史目录中没有可计算的季度文件")
    return min(item[0] for item in ranges), max(item[1] for item in ranges)


def _broker_observation_ranges(
    uploaded_file: str | Path,
    ...
    """Use the uploaded year start through its maximum business date."""
    uploaded_start, uploaded_end = _uploaded_observation_range(uploaded_file)
    history_start, history_end = _history_observation_range(history_files)
    source_start, source_end = _event_observation_range(event_manifest)
    effective_start = date(uploaded_end.year, 1, 1)
    effective_end = uploaded_end
    _require_event_range(event_manifest, effective_start, effective_end)
    if history_start > effective_start or history_end < effective_end:
        raise ValueError(
            "02 历史季度文件未完整覆盖年初至上传截止日范围："
            f"历史目录={history_start.isoformat()}—{history_end.isoformat()}，"
            f"应覆盖={effective_start.isoformat()}—{effective_end.isoformat()}"
        )
    if observation_start not in (None, "") and _as_date(observation_start) != effective_start:
        raise ValueError(
            "02 观察开始日期必须等于上传文件所属年份的 1 月 1 日: "
            f"{effective_start.isoformat()}"
        )
    if observation_end not in (None, "") and _as_date(observation_end) != effective_end:
        raise ValueError(
            "02 观察结束日期必须等于上传文件实际最大业务日期: "
            f"{effective_end.isoformat()}"
        )
    uncovered = []
    if history_start < effective_start:
        uncovered.append(
            {
                "start": history_start.isoformat(),
                "end": (effective_start - timedelta(days=1)).isoformat(),
            }
        )
    if effective_end < history_end:
        uncovered.append(
            {
                "start": (effective_end + timedelta(days=1)).isoformat(),
                "end": history_end.isoformat(),
            }
        )
    return {
        "source_event_start": source_start.isoformat(),
        "source_event_end": source_end.isoformat(),
        "uploaded_start": uploaded_start.isoformat(),
        "uploaded_end": uploaded_end.isoformat(),
        "history_start": history_start.isoformat(),
        "history_end": history_end.isoformat(),
        "effective_start": effective_start.isoformat(),
        "effective_end": effective_end.isoformat(),
        "history_ranges_outside_calculation": uncovered,
        "source_event_covers_entire_history_range": not uncovered,
    }
...
def _run_broker(parameters: dict[str, Any], *, peer: bool, project_root: Path) -> dict[str, Any]:
    ...
    observation_ranges = _broker_observation_ranges(
        parameters["uploaded_history_file"],
        parameters["event_manifest"],
        parameters["history_files"],
        parameters.get("observation_start"),
        parameters.get("observation_end"),
    )
    observation_start = date.fromisoformat(observation_ranges["effective_start"])
    observation_end = date.fromisoformat(observation_ranges["effective_end"])
    result = run_real_broker_metrics(
        ...
        observation_range_context=observation_ranges,
        compact_output=True,
    )
