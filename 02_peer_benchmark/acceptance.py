...
TASK_TYPE = 'peer-benchmark'
OUTPUT_CONTRACT = ('event_broker_assessments.csv', '券商预警指标最终汇总.csv')
OUTPUT_CONTRACTS = {'peer-benchmark': ('event_broker_assessments.csv', '券商预警指标最终汇总.csv'), 'single-model': ('event_broker_assessments.csv', '券商预警指标最终汇总.csv', '券商03A-G档位风险事件汇总.csv', '券商03A-G档位风险事件明细.csv')}
...
def _verify_config() -> None:
    for name, expected in (("task.all.json", TASK_TYPE), ("task.single.json", "single-model")):
        payload = json.loads((ROOT / name).read_text(encoding="utf-8"))
        if payload.get("task") != expected:
            raise SystemExit(f"{name} task mismatch: {payload.get('task')}")


def _verify_import() -> str:
    from riskaudit.broker_metrics.real_run import run_real_broker_metrics
    target = run_real_broker_metrics
...
def _verify_results(result_dir: Path) -> dict[str, object]:
    ...
    if not manifest_path.is_file() and TASK_TYPE == "peer-benchmark":
        manifest_path = result_dir / "核对材料" / manifest_name
    ...
    file_name_map = payload.get("file_name_map", {})
    paths = {name: result_dir / file_name_map.get(name, name) for name in output_contract}
